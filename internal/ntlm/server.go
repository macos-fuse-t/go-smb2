...
func (s *Server) Challenge(nmsg []byte) (cmsg []byte, err error) {
	...
	if len(s.nbDomain) > 0 {
		ti = append(TargetInfo{AvPair{MsvAvNbDomainName, utf16le.EncodeStringToBytes(s.nbDomain)}}, ti...)
	}
	if len(s.nbName) > 0 {
		ti = append(TargetInfo{AvPair{MsvAvNbComputerName, utf16le.EncodeStringToBytes(s.nbName)}}, ti...)
	}
	if len(s.dnsDomain) > 0 {
		ti = append(TargetInfo{AvPair{MsvAvDnsDomainName, utf16le.EncodeStringToBytes(s.dnsDomain)}}, ti...)
	}
	if len(s.dnsName) > 0 {
		ti = append(TargetInfo{AvPair{MsvAvDnsComputerName, utf16le.EncodeStringToBytes(s.dnsName)}}, ti...)
	}
	b := [8]byte{}
	t := smb2.NsecToFiletime(time.Now().UnixNano())
	t.Encode(b[:])
	ti = append(TargetInfo{AvPair{MsvAvTimestamp, b[:]}}, ti...)
