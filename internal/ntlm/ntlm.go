...
func (i *targetInfoEncoder) encode(dst []byte) {
	...
	if flags, ok := i.InfoMap[MsvAvFlags]; ok {
		...
		le.PutUint16(dst[off:off+2], uint16(MsvAvFlags))
	...
	le.PutUint16(dst[off:off+2], uint16(MsvAvChannelBindings))
	le.PutUint16(dst[off+2:off+4], 16)

	off += 20

	if len(i.SPN) != 0 {
		le.PutUint16(dst[off:off+2], uint16(MsvAvTargetName))
	...
	le.PutUint16(dst[off:off+2], uint16(MsvAvEOL))
...
func parseAvPairs(bs []byte) (pairs map[AvPairType][]byte, ok bool) {
	//        AvPair
	//   0-2: AvId
	//   2-4: AvLen
	//    4-: Value
	...
	pairs = make(map[AvPairType][]byte)
...
func (t *TargetInfo) Encode(dst []byte) {
	off := 0
	for _, p := range *t {
		le.PutUint16(dst[off:], uint16(p.AvID))
		le.PutUint16(dst[off+2:], uint16(len(p.Value)))
		off += 4
