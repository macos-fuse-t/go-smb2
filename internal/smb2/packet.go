...
func (p PacketCodec) IsCompound() bool {
	return ((p.Flags() & SMB2_FLAGS_RELATED_OPERATIONS) == SMB2_FLAGS_RELATED_OPERATIONS) || p.NextCommand() != 0
}

func (p PacketCodec) IsCompoundFirst() bool {
	return ((p.Flags() & SMB2_FLAGS_RELATED_OPERATIONS) == 0) && p.NextCommand() != 0
}

func (p PacketCodec) IsCompoundLast() bool {
	return ((p.Flags() & SMB2_FLAGS_RELATED_OPERATIONS) == SMB2_FLAGS_RELATED_OPERATIONS) && p.NextCommand() == 0
}
