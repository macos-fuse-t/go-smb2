...
func (r CreateContext) Size() int {
	return Roundup(16+len(r.Name), 8) + r.Data.Size()
}

func (r CreateContext) Encode(pkt []byte) {
	off := Roundup(16+len(r.Name), 8)
	le.PutUint16(pkt[4:], 16)                     // NameOffset
	le.PutUint16(pkt[6:], uint16(len(r.Name)))    // NameLength
	le.PutUint16(pkt[10:], uint16(off))           // Data Offset
	le.PutUint32(pkt[12:], uint32(r.Data.Size())) // Data Length
	copy(pkt[16:], r.Name)
	r.Data.Encode(pkt[off:])
}

type CreateContextMaximalAccessResponse struct {
	QueryStatus   uint32
	MaximalAccess uint32
}

func (r CreateContextMaximalAccessResponse) Size() int {
	return 8
}

func (r CreateContextMaximalAccessResponse) Encode(pkt []byte) {
	le.PutUint32(pkt[0:], r.QueryStatus)
	le.PutUint32(pkt[4:], r.MaximalAccess)
}
...
func (c *CloseResponse) Encode(pkt []byte) {
	...
	le.PutUint16(res[2:4], c.CloseFlags)
	if c.CreationTime != nil {
		c.CreationTime.Encode(res[8:16])
	}
	if c.LastAccessTime != nil {
...
func (c *LockResponse) Header() *PacketHeader {
	return &c.PacketHeader
}
...
func (c *IoctlResponse) Encode(pkt []byte) {
	...
	le.PutUint32(res[40:44], c.Flags2)
...
func (c *ChangeNotifyResponse) Header() *PacketHeader {
	return &c.PacketHeader
}
...
const (
	AAPL_SERVER_CAPS = 0x01
	AAPL_VOLUME_CAPS = 0x02
	AAPL_MODEL_INFO  = 0x04
)

// Caps
const (
	AAPL_SUPPORTS_READDIR_ATTR = 0x01
	AAPL_SUPPORTS_OSX_COPYFILE = 0x02
	AAPL_UNIX_BASED            = 0x04
	AAPL_SUPPORTS_NFS_ACE      = 0x08
)

// Volume caps
const (
	AAPL_SUPPORT_RESOLVE_ID = 0x01
	AAPL_CASE_SENSITIVE     = 0x02
	AAPL_SUPPORTS_FULL_SYNC = 0x04
)

type AAPLServerQueryResponse struct {
	CommandCode uint32
	Reserved    uint32
	ReplyBitmap uint64
	ServerCaps  uint64
...
func (r *AAPLServerQueryResponse) Size() int {
	return Roundup(40+utf16le.EncodedStringLen(r.ModelString), 8)
}

func (r *AAPLServerQueryResponse) Encode(pkt []byte) {
	le.PutUint32(pkt[:], r.CommandCode)
	le.PutUint64(pkt[8:], r.ReplyBitmap)
	le.PutUint64(pkt[16:], r.ServerCaps)
	le.PutUint64(pkt[24:], r.VolumeCaps)
	le.PutUint32(pkt[36:], uint32(utf16le.EncodedStringLen(r.ModelString)))
	utf16le.EncodeString(pkt[40:], r.ModelString)
}

type DurableHandleResponse2 struct {
	Timeout uint32
	Flags   uint32
}

func (r *DurableHandleResponse2) Size() int {
	return 8
}

func (r *DurableHandleResponse2) Encode(pkt []byte) {
	le.PutUint32(pkt[:], r.Timeout)
	le.PutUint32(pkt[4:], r.Flags)
}
...
func (r *LeaseResponse) Size() int {
	return 32
}

func (r *LeaseResponse) Encode(pkt []byte) {
	copy(pkt[:], r.LeaseKey[:])
	le.PutUint32(pkt[16:], r.LeaseState)
	le.PutUint32(pkt[20:], r.LeaseFlags)
	le.PutUint64(pkt[24:], r.LeaseDuration)
}

type LeaseResponse2 struct {
	LeaseResponse
	ParentLeaseKey Guid
	Epoch          uint16
}

func (r *LeaseResponse2) Size() int {
	return 52
}

func (r *LeaseResponse2) Encode(pkt []byte) {
...
func (r *DiskIdResponse) Size() int {
	return 32
}
