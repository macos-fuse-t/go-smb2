...
func (fd *FileId) ToUUID() UUID {
	uuid := UUID{}
	copy(fd.Persistent[:], uuid[:])
	copy(fd.Volatile[:], uuid[8:])
	return uuid
}
...
func (c *CompressionContext) Size() int {
	return 16 + len(c.Compressions)*2
}

func (c *CompressionContext) Encode(p []byte) {
	le.PutUint16(p[:2], SMB2_COMPRESSION_CAPABILITIES)    // ContextType
	le.PutUint16(p[2:4], uint16(8+len(c.Compressions)*2)) // DataLength

	{
		...
		{ // Compressions
			bs := d[8:]
			for i, c := range c.Compressions {
				le.PutUint16(bs[2*i:2*i+2], c)
			}
			le.PutUint16(d[:], uint16(len(c.Compressions))) // Compression Count
			le.PutUint32(d[4:], c.Flags)
		}
	}
}
